//export lib vulnerability to /root/vuln.txt
...
for v in range(listMem.len - 1)
	...
	list.push("Scanning for vulnerabilities at memory zone: " + listMem[v - 1])
	list.push(meta.scan_address(metaLib, listMem[v - 1]))
end for

myfile = get_shell.host_computer.File("/root/vuln")
myfile.set_content(list)